...
export default {
    data: new SlashCommandBuilder()
        .setName('play')
        .setDescription('Speel een nummer af en laat de bot joinen')
        .addStringOption(option => 
            option.setName('nummer')
                .setDescription('De Spotify/YouTube link of naam van het nummer')
                .setRequired(true)),
    ...
    async execute(interaction) {
        ...
        if (!member.voice.channel) {
            return interaction.reply({ content: '❌ Je moet eerst in een spraakkanaal zitten!', ephemeral: true });
        }
        ...
        if (!queue) {
            queue = { connection: null, player: createAudioPlayer(), songs: [] };
        ...
        try {
            ...
            if (play.sp_validate(query) !== false) {
                ...
                if (searchResults.length === 0) return interaction.editReply('❌ Nummer niet gevonden op Spotify/YouTube.');
            ...
            if (!queue.connection) {
                ...
                await interaction.editReply(`🔊 Gejoind en nu aan het afspelen: **${title}**`);
            } else {
                await interaction.editReply(`➕ Toegevoegd aan wachtrij: **${title}**`);
            }

        } catch (error) {
            console.error(error);
            interaction.editReply('❌ Er ging iets mis bij het starten van de muziek.');
        }
    },
};
